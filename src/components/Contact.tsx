    ...
    emailjs.send(serviceId, templateId, templateParams, publicKey)
      .then(() => {
        ...
        setToast({
          type: 'success',
          title: 'Message Sent!',
          message: "Thank you for contacting VALO Tech.\nWe'll get back to you as soon as possible."
        });

        // Hide success screen and toast
      ...
      <AnimatePresence>
        {toast && (
          <motion.div
            ...
            className={`fixed bottom-6 right-6 z-50 max-w-sm w-full p-4 rounded-2xl glass-panel bg-white/70 shadow-2xl backdrop-blur-md flex gap-3 text-left border ${
              toast.type === 'success' ? 'border-white/80' : 'border-red-500/30'
            }`}
          >
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${
              toast.type === 'success' ? 'bg-secondary/10 text-secondary' : 'bg-red-500/10 text-red-500'
            }`}>
              {toast.type === 'success' ? (
            ...
            <div className="flex-1 space-y-1">
              ...
              <p className="text-[11px] font-semibold text-primary/70 whitespace-pre-line leading-normal">
                {toast.message}
              </p>
            </div>
            <button 
              onClick={() => setToast(null)}
              className="text-primary/40 hover:text-primary/70 transition-colors text-xs font-bold self-start"
            >
              ✕
            </button>
          </motion.div>
